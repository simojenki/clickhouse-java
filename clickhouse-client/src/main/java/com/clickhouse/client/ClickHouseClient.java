...
public interface ClickHouseClient extends AutoCloseable {
    ...
    default boolean ping(ClickHouseNode server, int timeout) {
        if (server != null) {
            ...
            ClickHouseRequest<?> request = new ClickHouseRequest<>(this, ClickHouseChecker.nonNull(server, "Node"),
                new AtomicReference<>(server), server.config.getAllOptions(), false);
